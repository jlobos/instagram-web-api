...
class Instagram {
  ...
  async getComments({ shortcode, childCommentCount, fetchCommentCount, parentCommentCount }) {
    const variables = JSON.stringify({
      shortcode: shortcode,
      child_comment_count: childCommentCount || 3,
      fetch_comment_count: fetchCommentCount || 40,
      parent_comment_count: parentCommentCount || 24,
      has_threaded_comments: true
    })
    const options = {
      qs: {
        query_hash: 'a92f76e852576a71d3d7cef4c033a95e',
        variables
      }
    }
    ...
    return this.request
      ...
      .then(response => {
        const { data: { shortcode_media: { id, shortcode, edge_media_to_parent_comment } } } = response;
        return { id, shortcode, edge_media_to_parent_comment };
      })
