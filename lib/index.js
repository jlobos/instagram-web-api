...
class Instagram {
  ...
  async _uploadPhoto({ photo }) {
    ...
    const dimensionsImage = imageSize(photo);
    ...
    let ext = photo.lastIndexOf('.');
    ext = (ext < 0) ? '' : photo.substr(ext);
  ...
  async uploadPhoto({ photo, caption = '', configureLocale = 'feed'}) {
    ...
    return this.request.post((configureLocale === 'feed' ? '/create/configure/' : '/create/configure_to_story/'), {
      form: {
  ...
  async getUserMediaTagged({userId, first, after}){
