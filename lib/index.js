...
module.exports = class Instagram {
  ...
  async login({ username, password } = {}) {
    ...
    // Scraping in '/' and get CSRFToken from "_sharedData" before login
    const _sharedData = await this._getSharedData()

    // Provide CSRFToken for login or challenge request
    this.request = this.request.defaults({
      headers: { 'X-CSRFToken': _sharedData.config.csrf_token }
    })
    ...
    // Provide "_sharedData" after of login
  ...
  async getHastagPhotos({ hashtag, first = 12, after = '' } = {}) {
    return this.request('/graphql/query/', {
      qs: {
