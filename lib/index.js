...
class Instagram {
  ...
  async _uploadPhoto({ photo }) {
    ...
    if(isUrl(photo)){
      ...
      await axios( { url: photo, method: 'GET', responseType: 'arraybuffer' }).then((response) => {
    ...
    // Validate image input/aspect ratio
    if(dimensionsImage.type !== 'jpg' || dimensionsImage.width < 1080 || dimensionsImage.height < 1080){

      const width = (dimensionsImage.width < 1080 ? 1080 : dimensionsImage.width);
      const height = (dimensionsImage.height < 1080 ? 1080 : dimensionsImage.height);

      // Resize image and convert to jpeg
      await sharp(file).resize({width: width, height: height}).toFormat('jpeg').toBuffer().then((responseConvert) => {
        file = responseConvert;
