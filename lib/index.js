...
const getTotp = require('totp-generator');

const baseUrl = 'https://www.instagram.com'

class Instagram {
  constructor(
    { username, password, cookieStore, twoFA },
  ...
  async login({ username, password, twoFA } = {}, { _sharedData = true } = {}) {
    username = username || this.credentials.username
    password = password || this.credentials.password
    twoFA = twoFA || this.credentials.twoFA
    ...
    try {
      ...
      if (error.message.includes('"two_factor_required":true')) {

        // Get identifier from previous request
        const pattern = new RegExp(/(identifier":")\w+/)
        const matches = error.message.match(pattern)
        value = matches[0].substring(13)

        // Convert twoFA to a 6 digit code if a secret key was provided
        twoFA = twoFA.length === 6 ? twoFA : getTotp(twoFA);

        // Send a login request containing the 2FA code
        res = await this.request.post('/accounts/login/ajax/two_factor/', {
          resolveWithFullResponse: true,
          form: { username, verificationCode: twoFA, identifier: value }
        })
      }
    }
