...
class Instagram {
  constructor(
    ...
    // Configure proxy axios
    const pxAxios = this._configureProxyAxios(proxy);
    axios.defaults.proxy = pxAxios;
  ...
  async login({ username, password } = {}, { _sharedData = true } = {}) {
    ...
    // Provide CSRFToken to axios request
  ...
  async _uploadPhoto({ photo }) {
    ...
    let resizeTemp = false;

    // Needed to new method, if image is url.
    if(isUrl(photo)){

      isTempFile = true;

      // Start write temp file
      const writer = fs.createWriteStream('./tempDownloadImage');

      // Download temp file
      const imageDownload = await axios({
        url: photo, method: 'GET', responseType: 'stream'
      });
      ...
      file = await fs.readFileSync('./tempDownloadImage');
    ...
    // Get dimensions of the image, and format
    let dimensionsImage = imageSize(file);

    // Check image dimensions and format, if not correct resize and convert to jpg
    if(dimensionsImage.width < 1080 || dimensionsImage.height < 1080 || dimensionsImage.type !== 'jpg'){

      // If not correct, resize image and convert to jpeg.
      await sharp((isTempFile === true ? './tempDownloadImage' : photo))
      .resize({ height: 1080, width: 1080 })
      .jpeg({quality: 100})
      .toFile('./resize_tempImage.jpg')
      .then(() => {

        // Update file read dimensionsImage
        file = fs.readFileSync('./resize_tempImage.jpg')
        dimensionsImage = imageSize(file);
        resizeTemp = true;
      })
    }


    // Get this.request Cookies to axios request
    let cookieAxios
    await this.request.get('/', { resolveWithFullResponse: true }).then((rs) => {
      cookieAxios = rs.req._header.split('\n')[8].replace('cookie: ', '').replace('\r', '');
    });

    const ruploadParams = {
      media_type: 1,
      upload_id: uploadId.toString(),
      upload_media_height: dimensionsImage.width || 1080,
      upload_media_width: dimensionsImage.height || 1080,
    ...
    const responseUpload = await axios({
      url: `${baseUrl}/rupload_igphoto/${nameEntity}`,
      method: 'POST',
    ...
    if(isTempFile === true)
      fs.unlinkSync('./tempDownloadImage')

    if(resizeTemp === true)
      fs.unlinkSync('./resize_tempImage.jpg');
  ...
  // ConfigureLocale: 'feed' or 'story'
  async uploadPhoto({ photo, caption = '', configureLocale = 'feed'}) {
    ...
    return this.request.post(`/create/${(configureLocale === 'feed' ? 'configure/' : 'configure_to_story/')}`, {
      form: {
  ...
  // Configure axios proxy support
  _configureProxyAxios(proxyAxios){

    let proxy = proxyAxios;
    ...
    if(proxy.search('@') > -1){
      ...
      const proxyAuth = proxy[0].split(':');
      const proxyHostPort = proxy[1].split(':');
      ...
      const proxyHostPort = proxy.split(':');
  ...
  async getUserMediaTagged({id, first = 12, after = ''}){
    return this.request('/graphql/query/', {
      qs: {
      ...
      .then(response => response.data.user.edge_user_to_photos_of_you || {})
      .then(({ count = 0, page_info = {}, edges = [] }) => ({
        count,
        page_info,
        edges
  ...
  async likeComment(commentId){
    return this.request.post(`/web/comments/like/${commentId}/`)
  }

  async unlikeComment(commentId){
    return this.request.post(`/web/comments/unlike/${commentId}/`)
  }
