...
class Instagram {
  constructor(
    { username, password, cookieStore, totp },
  ...
  async login({ username, password, totp } = {}, { _sharedData = true } = {}) {
    username = username || this.credentials.username
    password = password || this.credentials.password
    totp = totp || this.credentials.totp
    ...
    try {
      ...
      if (error.message.includes('"two_factor_required":true')) {
        ...
        res = await this.request.post('/accounts/login/ajax/two_factor/', {
          resolveWithFullResponse: true,
          form: { username, verificationCode: getTotp(totp), identifier: value }
        })
      }
    }
