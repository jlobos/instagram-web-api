...
class Instagram {
  constructor(
    ...
    let pxAxios = this._configureProxyAxios(proxy);
  ...
  async _uploadPhoto({ photo }) {
    ...
    // Validate file
    if(isUrl(photo))
      throw new Error('Image cannot be a url.')

    let file = await fs.readFileSync(photo);
    const dimensionsImage = imageSize(file);

    // Validate image aspect ratio/format
    if(dimensionsImage.width < 1080 || dimensionsImage.height < 1080)
      throw new Error(`Image isn't in an allowed aspect ratio.`);
    
    // Validate image extension
    if(dimensionsImage.type !== 'jpg')
      throw new Error('Image format not allowed!. Only jpeg images.');
    ...
    await this.request.get('/', { resolveWithFullResponse: true }).then((rs) => {
      ...
      cookieAxios = cookieAxios[8].replace('cookie: ', '');
      cookieAxios = cookieAxios.replace('\r','');
    });

    let ruploadParams = {
      media_type: 1,
      upload_id: uploadId.toString(),
      upload_media_height: dimensionsImage.width || 1080, //don't change this
      upload_media_width: dimensionsImage.height || 1080, //don't change this
  ...
  _configureProxyAxios(proxyAxios){
    ...
    if(proxy.search('@') > -1){
      ...
      let proxyHostPort = proxy[1].split(':');
  ...
  async getUserMediaTagged({id, first, after}){
